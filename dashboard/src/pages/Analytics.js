...
export default function AnalyticsPage() {
    ...
    const doughnutData = {
        ...
        datasets: [{
            data: [analytics.won, analytics.lost, analytics.void, analytics.pending],
    ...
    const kpis = [
        { label: 'ROI', value: `${analytics.roi.toFixed(2)}%`, color: analytics.roi >= 0 ? 'positive' : 'negative' },
        { label: 'Yield', value: `${analytics.yield_pct.toFixed(2)}%`, color: analytics.yield_pct >= 0 ? 'positive' : 'negative' },
        { label: 'Hit Rate', value: `${analytics.hit_rate.toFixed(1)}%`, color: 'neutral' },
        { label: 'Avg Odds', value: analytics.avg_odds.toFixed(3), color: 'neutral' },
        { label: 'Total Staked', value: analytics.total_staked.toFixed(2), color: 'neutral' },
        { label: 'Net Profit', value: `${analytics.total_profit >= 0 ? '+' : ''}${analytics.total_profit.toFixed(2)}`, color: analytics.total_profit >= 0 ? 'positive' : 'negative' },
    ];
